...
# 
# Shovel Core:
# 1. Write a class to take over for lots of if's on argv input
#
# 2. Remove the actual parsing from this class, seperate it so that 
#    it is easier to update
#
# 3. I think it would be quite interesting to support different versions of 
#    the parser, so say you add the metadata 0.0.1-dev or whatever and you 
#    have version 0.1.0-dev installed it would automatically parse it right.
#    because i am pretty convinced i will have major changes to the yaml
#    structures.
#
# 4. String parsing ie. {d:sandbox} or {d:sandbox:build:ffmpeg} or {i:os}
...
#  3. Support better namespacing and inheritance for the plugin names
# Downloader:
# [x] 1. Progress bar for urllib
#
# Patcher:
# [x] 1. Actually get this to work
# 2. I would like to have it automatically figure out the strip by aligning
#    the directories up (this really is doable)
#
# Builder:
# 1. Find a more elegant way for stderr to be run
# 2. Support options to enable/disable the parsing of gcc
#
#
# Utils:
# 1. Get pprint to multiline print properly
# 2. Consolidate all of the terminal utilities here
#
#
#
# Any code that has 
